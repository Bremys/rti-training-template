...
public class PlotTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "Plot";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final PlotTypeSupport _singleton
    = new PlotTypeSupport();
    ...
    */
    public static PlotTypeSupport get_instance() {
        return _singleton;
    }

    public static PlotTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return PlotTypeCode.VALUE;
    }
    ...
    * <code>this</code> is not a <code>Plot</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        Plot typedDst = (Plot) destination;
        Plot typedSrc = (Plot) source;
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeDouble(typedSrc.azimuth);

            dst.writeDouble(typedSrc.range);

            dst.writeDouble(typedSrc.elevation);

            dst.writeDouble(typedSrc.doppler);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            Plot typedDst = (Plot) dst;
            typedDst.clear();      
            try{
                typedDst.azimuth = src.readDouble();
                typedDst.range = src.readDouble();
                typedDst.elevation = src.readDouble();
                typedDst.doppler = src.readDouble();
    ...
    int mask) {
        return new PlotDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new PlotDataReader(native_reader, listener, mask, this);   
    ...
    protected PlotTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,PlotTypeCode.VALUE,Plot.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected PlotTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,PlotTypeCode.VALUE,Plot.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
